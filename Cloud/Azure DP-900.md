...
1\\ Администраторы баз данных (доступи, бекап, восстановление)
...
1.2\\ Azure SQL Managed Instance (PaaS) - для миграции с минимальными изменениями и интергация с сервисами. Обслуживает облачный провайдер.

1.3\\ SQL Server on Azure VM (IaaS) -  контроль над ресурсам, для "lift and shift". Сам установи, обновляй патчи, настраивай резервное копирование и восстановление.
...
Виды: Block blobs (макс 4000 MiB X 50,000 blocks, для больших объемов, не для частого изменеия частей файла); Page blobs (по 512 байт, на виртуальных жестких дисках, для случайного доступа с чтением и записью в любом месте); Append blobs (до 4 МБ только в конец без обновления или удаления).
...
**4\\ Azure Table Storage** — таблицы «ключ/значение» с разными столбцами и независимыми разделами. Все строки должны иметь ПК (состоящий из ключа раздела и ключа строки, нет ФК), при изменении данных записывает дату и время изменений. Доступ к данным через API (REST) и SDK.
...
Нереляционные базы данных (NoSQL): Key-value databases (значение любой формат, доступ по API и SDK); Document databases (значение в JSON); Column family databases; Графовые базы данных.

Time series database (NOSQL/SQL) — это специализированная база данных, предназначенная для хранения, обработки и анализа временных рядов данных. Временной ряд — это последовательность данных, отсортированных по времени (пр: показания датчиков, финансовые котировки, метрики серверов).
...
*Результат: {    "id": "joe@litware.com",    "name": "Joe Jones",    "address": {"street": "1 Main St.", "city": "Seattle"     } }*

**2\\** **Azure Cosmos DB for MongoDB** - данные в Binary JSON (BSON)

Применения: Хранение документов, данных для аналитики, событий; SaaS-платформы.

*ПР: запрос: db.products.find({id: 123})*

*Результат: {    "id": 123,    "name": "Hammer",    "price": 2.99 }*

**3\\ Azure Cosmos DB for PostgreSQL** - реляционная глобально распределенная БД.

Применения: Транзакции, отчёты, финансовые системы.

*ПР: SELECT ProductName, Price FROM Products WHERE ProductID = 123; Результат: таблица.*

**4\\** **Azure Cosmos DB for Table** - данные типа «ключ-значение» (аналогично Azure Table Storage). Доступ к данным через API (REST) и SDK.

Применения: Бэкапы, архивы, мониторинг; Логи, телеметрия, события IoT.

*ПР: https://endpoint/Customers(PartitionKey='1',RowKey='124')*

**5\\ Azure Cosmos DB for Apache Cassandra** - данные на основе семейств (разных) столбцов. Пары столбец-значение(=набор столбцов). Применения: Приложения с высокой скоростью записи и чтения; Логи, телеметрия, события IoT.

*ПР: SELECT \* FROM Employees WHERE ID = 2   Результат: таблица, где значения не у всех столбцах.*

**6\\ Azure Cosmos DB for Apache Gremlin** - данные в графовой структуре (сущности=вершины, узлы соединены ребрами, представляющими отношения).
...
Куб — информация организована по нескольким измерениям (пр: время, продукт, регион).
...
Елементы: linked service, pipeline, dataset, activities, dataflow.
...
Delta Lake —  надстройка (management layer) над хранилищем данных, которая обеспечивает ACID-транзакции, версионирование данных, управление схемой поверх хранения файлов в формате Parquet, метаданные для отслеживания состояния таблиц. Delta Lake вместе со Spark позволяет удобно хранить и обрабатывать как потоковые, так и пакетные данные в виде таблиц, к которым можно делать SQL-запросы для анализа.
...
**6\\** **HDInsight** используется для обработки больших объёмов данных с помощью Hadoop.

**7\\** **Azure Synapse Analytics** — это собственный сервис Azure, построенный на Apache Spark.
...
Иерархия позволяет выполнять детализацию вверх и вниз внутри измерения.

Scatter plots - для выявления связи или корреляции между двумя числовыми показателями.

Pie charts наглядно сравнивают значения как доли от общего количества.

Line charts показывают динамику и тенденции, обычно во времени.

Bar\&Column charts применяются для сравнения значений по отдельным категориям.

Card - для отслеживания отдельного показателя.

Матрица - разворачивает таблицу по измерениям.
